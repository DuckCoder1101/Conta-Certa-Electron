...
import { BillingCadDTO, BillingResumeDTO, BillingWithTotalFee, IAppResponse } from '@app/shared-types';
...
export async function FetchBillingsController(
  ...
  limit = 30,
  filter = '',
): Promise<IAppResponse<BillingWithTotalFee[]>> {
  try {
...
export async function FetchBillingsResumeController(): Promise<IAppResponse<BillingResumeDTO[]>> {
  try {
...
export async function SaveBillingController(_event: IpcMainInvokeEvent, billing: BillingCadDTO): Promise<IAppResponse> {
  try {
