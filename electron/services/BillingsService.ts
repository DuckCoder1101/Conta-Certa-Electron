...
import { BillingCadDTO, BillingResumeDTO, BillingStatus, BillingWithTotalFee } from '@app/shared-types';
...
export async function FetchBillingsWithTotalService(offset: number, limit: number, filter: string) {
    ...
    where: {
      AND: [
        filter
          ? {
              OR: [
                { client: { name: { startsWith: filter } } },
                { status: filter === 'pendente' ? 'pending' : filter === 'pago' ? 'paid' : '' },
              ],
            }
          : {},
      ],
  ...
  const billings: BillingWithTotalFee[] = billingsRaw.map((b) => ({
    ...b,
...
export async function FetchBillingsResumeService() {
  ...
  const billings: BillingResumeDTO[] = billingsRaw.map((b) => ({
    fee: b.fee,
...
export async function CreateBillingService(billing: BillingCadDTO) {
...
export async function EditBillingService(billing: BillingCadDTO) {
